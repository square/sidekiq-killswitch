  ...
  module Killswitch
    ...
    class << self
      ...
      def blackhole_worker?(worker_name)
        redis_pool do |redis|
          hexists = redis.hexists(BLACKHOLE_WORKERS_KEY_NAME, class_to_str(worker_name))
          hexists == 1 || hexists == true
        end
      end
      ...
      def dead_queue_worker?(worker_name)
        redis_pool do |redis|
          hexists = redis.hexists(DEAD_QUEUE_WORKERS_KEY_NAME, class_to_str(worker_name))
          hexists == 1 || hexists == true
        end
      end
